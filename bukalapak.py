...
def get_link_(numofpage,keywords):

	driver = webdriver.Chrome(ChromeDriverManager().install())
	driver.maximize_window()
	driver.get("https://www.bukalapak.com/")
	driver.find_element(By.XPATH, "//input[contains(@id,'search')]").send_keys(keywords)
	...
	product_link, allData_product = [], []

	for page_number in range(numofpage):
		Product_info_list = driver.find_element(By.XPATH, "//p[contains(@class,'bl-text bl-text--body-small bl-text--ellipsis__2')]")
		for findlink in Product_info_list:
			a = findlink.find_element_by_tag_name('a')
			product_link.append(a.get_property('href'))
		print("[INFO] page: {}; recorded product num: {}".format(page_number+1, len(product_link)))
...
data_scraping = get_link_(numofpage=1,keywords='laptop')
